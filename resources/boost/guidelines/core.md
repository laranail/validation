...
- The package also ships 48 domain rules under `Simtabi\Laranail\Validation\Rules\`, for formats Laravel has no rule for. They are plain `ValidationRule` objects: `['account' => ['required', new Iban()]]`, or in a chain via `->rule(new Iban())`.
